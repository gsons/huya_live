...
class huya_danmu extends events {
    ...
    async _get_chat_info() {
        ...
        try {
            ...
            let TT_ROOM_DATA = body.match(/var TT_ROOM_DATA = (.*?);/ig);
            let TT_PROFILE_INFO = body.match(/var TT_PROFILE_INFO = (.*?);/ig);
            body=TT_ROOM_DATA[0]+' '+TT_PROFILE_INFO[0];  
            console.log(body);            
            if (body) {
                let info = {}
                eval(body + "info.subsid = TT_ROOM_DATA.sid;info.topsid=TT_ROOM_DATA.id;info.yyuid=TT_PROFILE_INFO.lp")
                return info
            }
        } catch (e) { }
    }
